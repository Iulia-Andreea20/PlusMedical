...
const AdminDashboard = () => {
    ...
    const getStatusColor = (status) => {
      ...
      const handleApproveSubmit = async () => {
        // Logică pentru a trimite suma aprobată către server
      };
    
      const handleRejectSubmit = async () => {
        // Logică pentru a trimite motivul refuzului către server
      };
  ...
  return (
    <>
      ...
      <Container sx={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 64px)', mb: 8 }}> {/* Adaugă marja de jos pentru a nu se suprapune cu footer-ul */}
              ...
              <CardContent>
                <Grid container spacing={2}>
                  ...
                  {selectedDocument && (
                    <Grid item xs={12} md={6}>
                      <Box>
                        <iframe src={selectedDocument} width="100%" height="800px" />
                      </Box>
                    </Grid>
                  )}
                </Grid>
